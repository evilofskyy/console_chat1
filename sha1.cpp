...
uint* sha1(char* message, uint msize_bytes) {
    ...
    *ptr_to_size = bring_to_human_view(msize_bytes * 8);
    ...
    for (uint i = 0; i < totalBlockCount; i++) {
        ...
        for (uint j = 0; j < block_expend_size_uints; j++) {
            uint f, k;
            if (j < 20) {
                f = (b & c) | ((~b) & d);
                k = 0x5A827999;
    ...
    digest[0] = bring_to_human_view(A);
    digest[1] = bring_to_human_view(B);
    digest[2] = bring_to_human_view(C);
    digest[3] = bring_to_human_view(D);
    digest[4] = bring_to_human_view(E);
